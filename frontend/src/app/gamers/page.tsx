    ...
    <div className="mx-auto max-w-7xl px-4 py-8">
      ...
      {filtered.length === 0 ? (
        <EmptyState
          ...
          title="No gamers match that search"
          description="Try a different gamer tag, location, or platform."
          action={{ label: "Clear search", href: "/gamers" }}
        />
      ) : <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">{filtered.map((gamer) => (
        <Link key={gamer.id} href={"/profiles/" + encodeURIComponent(gamer.gamer_tag)} className="group rounded-2xl border border-ggz-border bg-ggz-bg-1 p-5 transition hover:-translate-y-0.5 hover:border-ggz-amber/40">
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1"><div className="flex items-center gap-2"><h2 className="truncate font-semibold text-ggz-text-primary group-hover:text-ggz-amber">{gamer.gamer_tag}</h2>{gamer.presence === "online" && <span className="relative flex h-2 w-2"><span className="live-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400" /><span className="relative inline-flex h-2 w-2 rounded-full bg-emerald-400" /></span>}</div><p className="mt-1 text-xs text-ggz-text-secondary">{gamer.location || "Location hidden"} · {gamer.platform || "Platform not set"}</p></div>
          </div>
