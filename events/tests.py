...
class EventManagementTests(TestCase):
	def setUp(self):
		self.user = User.objects.create_user(username="event-owner", password="pass", is_staff=True)
	...
	def test_non_staff_user_cannot_access_admin_organization_and_event_management(self):
	...
	def test_staff_can_activate_a_valid_saved_location_and_reject_missing_coordinates(self):
		self.user.is_staff = True
		self.user.save(update_fields=("is_staff",))
		self.client.login(username="event-owner", password="pass")
		org = Organization.objects.create(owner=self.profile, name="Map Ready", slug="map-ready", organization_type="Venue")
		location = org.locations.create(name="Map Ready Hub", city="Harare", latitude=-17.8252, longitude=31.0335, public_visible=False)
		...
		incomplete = org.locations.create(name="Missing Coordinates", city="Harare", public_visible=False)
		response = self.client.post(reverse("organization_location_visibility", args=(org.slug, incomplete.id, "activate")))
		self.assertEqual(response.status_code, 302)
